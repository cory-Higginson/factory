...
const int chunksize = 128;                     // chunk size
const int total_blocks = chunksize * chunksize;//  total blocks in one chunk
...
public:
    ...
    std::array<std::tuple<int, Block *> *, total_blocks> block_data;      ///< [in,out] @details array cointaining all special blocks @example chests,conveyors,factorys ect
};
